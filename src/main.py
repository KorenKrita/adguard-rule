...
from src.config import load_config, get_filter_urls, get_whitelist_urls, get_whitelist_rules, get_dns_urls
...
def main():
    ...
    if filter_sources:
        ...
        header = generate_header("Merged Filter", len(rules), stats)
        write_output(output_dir / "filter.txt", header, rules)
        print(f"  -> {len(rules)} rules written to output/filter.txt")
    else:
